...
static int hf_msgpack_double = -1;
static int hf_msgpack_ext_ext = -1;
...
static void dissect_msgpack_integer(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, guint8 type, void *data, int *offset, char **value)
{
    ...
    if (type >> 5 == 7) {
        proto_tree_add_int_format(tree, hf_msgpack_int_8, tvb, *offset, 1, type, "%s: %d", label, type);
        if (value)
    ...
    case 0xcd:
        ...
        proto_tree_add_uint_format(tree, hf_msgpack_uint_16, tvb, *offset, 3, uint16, "%s: %u", label, uint16);
        if (value)
    ...
    case 0xd0:
        ...
        proto_tree_add_int_format(tree, hf_msgpack_int_8, tvb, *offset, 2, int8, "%s: %d", label, int8);
        if (value)
    ...
    case 0xd1:
        ...
        proto_tree_add_int_format(tree, hf_msgpack_int_16, tvb, *offset, 3, int16, "%s: %d", label, int16);
        if (value)
...
static void dissect_msgpack_map(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, guint8 type, void *data, int *offset, char **value)
{
    ...
    ti = proto_tree_add_string_format(tree, hf_msgpack_string, tvb, *offset, lensize, NULL, "%s", label);
    subtree = proto_item_add_subtree(ti, ett_msgpack_map);
    *offset += lensize;

    for (guint32 i = 0; i < len; i++) {
        map_subtree = proto_item_add_subtree(subtree, ett_msgpack_map_elem);
...
static void dissect_msgpack_ext(tvbuff_t *tvb, proto_tree *tree, int type, void *data, int *offset, char **value)
{
    ...
    if (type >= 0xd4 && type <= 0xd8) {
        ...
        *offset += bytes;
        ...
        proto_tree_add_item(ext_tree, hf_msgpack_ext_ext, tvb, *offset, 1, ENC_NA);
        *offset += 1;
        ...
        case 0xc7:
            ...
            proto_tree_add_item(ext_tree, hf_msgpack_uint_8, tvb, *offset, 1, ENC_NA);
            *offset += 1;
            break;
        case 0xc8:
            len = tvb_get_ntohs(tvb, *offset);
            proto_tree_add_item(ext_tree, hf_msgpack_uint_16, tvb, *offset, 2, ENC_NA);
            *offset += 2;
            break;
        case 0xc9:
            len = tvb_get_ntohl(tvb, *offset);
            proto_tree_add_item(ext_tree, hf_msgpack_uint_32, tvb, *offset, 4, ENC_NA);
            *offset += 4;
            break;
        }
        ...
        *offset += 1;
        ...
        start = (const guint8 *)tvb_get_ptr(tvb, *offset, len);
        proto_tree_add_bytes(ext_tree, hf_msgpack_ext_bytes, tvb, *offset, len, start);
        if (value)
            *value = bytes_to_hexstr(*value, start, len);
        *offset += len;
    }
...
static void dissect_msgpack_object(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data, int *offset, char **value)
{
    ...
    // Integer
    if (type <= 0x7f || type >= 0xe0 || (type >= 0xcc && type <= 0xd3)) {
    ...
    // Double
    if (type == 0xcb) {
        dissect_msgpack_double(tvb, pinfo, tree, type, data, offset, value);
        return;
    }
...
void proto_register_msgpack(void)
{
    ...
    static hf_register_info hf[] = {
            ...
            { &hf_msgpack_ext_ext,
                    { "Ext", "msgpack.ext.ext", FT_UINT8, BASE_HEX, NULL, 0x00, NULL, HFILL }
            },
            { &hf_msgpack_ext_type,
                    { "Type", "msgpack.ext.type", FT_INT8, BASE_DEC, NULL, 0x00, NULL, HFILL }
            },
